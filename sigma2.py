...
ALPHA4_CFG = {
    ...
    'USE_RS_SMA_FILTER': True,   # Active le filtre de vélocité RS
    ...
    # --- NOUVEAU : Protection si marché repasse sous SMA100 pendant le trade ---
    'USE_BEAR_MKT_PROTECT': True,   # True = active la protection en trade
    'BEAR_MKT_CUT_TYPE': 'MKT_CUT', # Libellé de sortie si trade coupé sous son BE
    ...
    'UNIVERSE': None,        # Optionnel: restreindre l’univers ; None => tous
}
...
def _v4_run_ticker(stock_df: pd.DataFrame,
    ...
    # --- Séries marché alignées sur les dates du titre ---
    ...
    for i, date in enumerate(stock_df.index[start_i:]):
        ...
        if active_trade is not None:
            ...
            # ==========================================================
            # NOUVEAU : Protection si CAC40 < SMA100 pendant le trade
            # - si trade sous son BE   -> on coupe immédiatement au close
            # - si trade au-dessus BE  -> on force le SL à BE
            # Désactivable via USE_BEAR_MKT_PROTECT
            # ==========================================================
            bear_mkt_now = (
                cfg.get('USE_BEAR_MKT_PROTECT', False)
                and pd.notna(idx_close_on_stock_dates.loc[date])
                and pd.notna(idx_sma_stock_dates.loc[date])
                and (idx_close_on_stock_dates.loc[date] < idx_sma_stock_dates.loc[date])
            )

            if bear_mkt_now:
                # "Au-dessus de son BE" interprété ici au CLOSE
                above_be_now = active_trade['be_hit'] or (c_perf >= active_trade['be_trig'])

                if above_be_now:
                    # On force le SL à BE immédiatement
                    active_trade['be_hit'] = True
                else:
                    # Sous le BE : coupure immédiate au close
                    raw_exit = c_perf
